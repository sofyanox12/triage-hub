...
/**
 * Publishes a ticket update event to Redis.
 * @param payload - The update payload (status, analysis results).
 */
export const publishTicketUpdate = async (payload: TicketUpdatePayload) => {
    try {
        ...
        logger.info({ ticketId: payload.ticketId }, 'Published ticket update')
    } catch (error) {
